...
const FleetListPage: React.FC = () => {
  ...
  const getStatusStyles = (status: string) => {
    switch (status) {
      ...
      case 'unavailable': return 'bg-rose-50 text-rose-600 border-rose-200';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col shadow-sm min-h-[60vh]">
        <div className="bg-gray-50 px-8 pt-4 border-b border-gray-100 flex items-center gap-8">
          {(['all', 'active', 'maintenance', 'unavailable'] as const).map((s) => (
             <button
               ...
               className={cn(
                 ...
                 filter === s ? "text-indigo-600 border-indigo-600" : "text-gray-500 hover:text-gray-900 border-transparent"
               )}
             >
               {s}
             </button>
          ...
          <div className="overflow-x-auto">
                  ...
                  >
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-bold text-gray-900 group-hover:text-indigo-600 transition-colors uppercase tracking-widest">{vehicle.plateNo}</p>
                          <p className="text-[10px] text-gray-400 font-mono italic">{vehicle.make} {vehicle.model}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                       <span className="text-xs text-gray-700 uppercase font-black tracking-tighter">{vehicle.type}</span>
                       <div className="flex items-center gap-2 mt-1">
                          <Fuel className="w-3 h-3 text-amber-600" />
                          <span className="text-[10px] text-gray-400 uppercase font-bold">{vehicle.fuelType}</span>
                       </div>
                    </td>
                    <td className="px-8 py-5">
                       <span className={cn(
                         "px-2 py-0.5 rounded text-[10px] uppercase font-black tracking-widest border",
                         vehicle.ownership === 'owned' ? "border-emerald-200 text-emerald-600 bg-emerald-50" : "border-purple-200 text-purple-600 bg-purple-50"
                       )}>
                         {vehicle.ownership}
                       </span>
                    </td>
                    ...
                    <td className="px-8 py-5 text-right" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-end space-x-1 opacity-0 group-hover:opacity-100 transition-all">
                        <PermissionGate permission="edit_fleet">
                          <Link to={`/fleet/${vehicle.id}/edit`} className="p-2 text-gray-400 hover:text-gray-900 transition-colors">
            ...
            {filteredVehicles.length === 0 && (
              <div className="py-24 text-center">
                 <p className="text-gray-400 text-sm font-bold uppercase tracking-widest">No vehicles found matching criteria.</p>
              </div>
            )}
          </div>
        )}
